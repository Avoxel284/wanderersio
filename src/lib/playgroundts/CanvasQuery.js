...
cq.Layer.prototype = {
	...
	fillText: function (text, x, y, gap) {
		...
		var webkitHack =
			!cq.smoothing && this.fontHeight() <= 64 && "WebkitAppearance" in document.documentElement.style;
	...
	wrappedText: function (text, x, y, maxWidth, lineHeight) {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			if (textAlign === "left" || textAlign === "start") this.fillText(text, x, oy);
			else if (textAlign === "center") this.fillText(text, (x - width * 0.5) | 0, oy);
	...
	writeMeta: function (data) {
		...
		while (bytes.length) {
			...
			if (!bytes.length) for (var i = 0; i < 3 - pixel.length; i++) pixel.unshift(254);
